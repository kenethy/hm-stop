#!/bin/bash
# update-app-with-cancel-fix.sh - Script untuk memperbarui aplikasi dengan perbaikan untuk kasus pembatalan servis
...
echo -e "${YELLOW}Memulai pembaruan aplikasi dengan perbaikan untuk kasus pembatalan servis...${NC}\n"
...
cat > rebuild-mechanic-reports-with-cancel-fix.php << 'EOL'
<?php

// Script untuk membangun ulang rekap montir dengan perbaikan untuk kasus pembatalan servis
...
// Mulai log
Log::info("=== REBUILD: Memulai pembangunan ulang rekap montir dengan perbaikan untuk kasus pembatalan servis ===");
...
Log::info("=== REBUILD: Pembangunan ulang rekap montir dengan perbaikan untuk kasus pembatalan servis selesai ===");

echo "Pembangunan ulang rekap montir dengan perbaikan untuk kasus pembatalan servis selesai. {$count} servis diproses.\n";
EOL
...
docker cp rebuild-mechanic-reports-with-cancel-fix.php $(docker-compose ps -q app):/var/www/html/
if [ $? -ne 0 ]; then
...
docker-compose exec -T app php rebuild-mechanic-reports-with-cancel-fix.php
if [ $? -ne 0 ]; then
...
echo -e "\n${GREEN}Pembaruan aplikasi dengan perbaikan untuk kasus pembatalan servis selesai!${NC}"
