...
echo "=== MEMPERBAIKI MASALAH BIAYA JASA MONTIR ==="
...
# Langkah 2: Perbaiki metode calculateWeeklyLaborCost di Mechanic.php
echo ""
...
echo "Memperbaiki metode calculateWeeklyLaborCost..."
docker exec $CONTAINER_NAME bash -c 'cat > /tmp/fix_calculate_labor_cost.php << "EOF"
<?php

// Baca file Mechanic.php
$file = file_get_contents("app/Models/Mechanic.php");

// Ganti metode calculateWeeklyLaborCost
$pattern = '/public function calculateWeeklyLaborCost\(\$weekStart, \$weekEnd\)[^}]+}/s';
$replacement = 'public function calculateWeeklyLaborCost($weekStart, $weekEnd)
    {
        // Log untuk debugging
...
# Jalankan script PHP
docker exec $CONTAINER_NAME php /tmp/fix_calculate_labor_cost.php
