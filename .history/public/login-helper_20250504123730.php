...
if ($auth->attempt(['email' => $email, 'password' => $password])) {
    ...
    echo "<p>CSRF Token: " . $session->token() . "</p>";
    
    // Create a direct link to the admin panel with the session ID
    $adminUrl = url('/admin');
    ...
    setcookie('laravel_session', $session->getId(), time() + 3600, '/', null, false, true);
